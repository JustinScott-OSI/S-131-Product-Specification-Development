  ...
  local label
    ...
    if contextParameters.RadarOverlay then
        featurePortrayal:AddInstructions(
            'ViewingGroup:32440;DrawingPriority:15;DisplayPlane:OverRADAR'
        )
    else
        featurePortrayal:AddInstructions(
            'ViewingGroup:32440;DrawingPriority:15;DisplayPlane:UnderRADAR'
        )
    end
    ...
    if feature.bollardNumber and feature.bollardNumber[1] then
        if feature.bollardNumber[2] then
            label = (label and (label .. ' ')) or ''
            label = label .. 'Bol ' ..
                    feature.bollardNumber[1] .. '-' ..
                    feature.bollardNumber[2]
        else
            label = (label and (label .. ' ')) or ''
            label = label .. 'Bol ' .. feature.bollardNumber[1]
        end

    elseif feature.metreMarkNumber and feature.metreMarkNumber[1] then
        label = (label and (label .. ' ')) or ''
        if feature.metreMarkNumber[2] then
            label = label .. 'MM ' ..
                    feature.metreMarkNumber[1] .. '-' ..
                    feature.metreMarkNumber[2]
        else
            label = label .. 'MM ' .. feature.metreMarkNumber[1]
        end

    elseif feature.manifoldNumber and feature.manifoldNumber[1] then
        label = (label and (label .. ' ')) or ''
        if feature.manifoldNumber[2] then
            label = label .. 'Mnf ' ..
                    feature.manifoldNumber[1] .. '-' ..
                    feature.manifoldNumber[2]
        else
            label = label .. 'Mnf ' .. feature.manifoldNumber[1]
        end

    elseif feature.rampNumber and feature.rampNumber[1] then
        label = (label and (label .. ' ')) or ''
        label = label .. 'Rp ' .. feature.rampNumber[1]
    end

    -- =====================
    -- TEXT PORTRAYAL (CURVE-SAFE)
    -- =====================
    if label and label ~= '' then
        ...
        featurePortrayal:AddTextInstruction(
            EncodeString(label),
            29,       -- text viewing group
            24,       -- text priority
            32440,    -- feature viewing group
            15        -- feature priority
        )
    end
